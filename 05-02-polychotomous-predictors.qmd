...
# Read in data
pew = read_csv(file = "https://raw.githubusercontent.com/zief0002/modeling/main/data/pew2.csv")
...
#| fig-cap: "LEFT: Boxplots of the news knowledge versus news source. The jittered points show the individual data values."
#| fig-alt: "LEFT: Boxplots of the news knowledge versus news source. The jittered points show the individual data values."
#| fig-width: 8
#| fig-height: 4
#| out-width: "70%"
# Scatterplot
...
Below we write the syntax to create all four dummy variables and save the new columns in the `pew` data frame.
...
```{r}
#| label: tbl-dummies
#| tbl-cap: "News source and values on the four dummy-coded indicator variables for a sample of six people."
#| echo: false
#| results: asis
set.seed(35)

pew |>
  select(news_source, none, con, com, lib) |>
  sample_n(6) |>
  gt() |>
  cols_align(
    align = "left",
...
$$
\begin{split}
\hat{\mathrm{Knowledge}_i} = &48.7 + 7.8(\mathrm{Con}_i) + 15.0(\mathrm{Com}_i) + 16.9(\mathrm{Lib}_i) 
\end{split}
$$
...
$$
\begin{split}
H_0: &\bigg(\mu_{\mathrm{Con}} - \mu_{\mathrm{Com}}\bigg) = \bigg(\mu_{\mathrm{Con}} - \mu_{\mathrm{Lib}}\bigg) =\\ 
&\bigg(\mu_{\mathrm{Con}} - \mu_{\mathrm{None}}\bigg) = \bigg(\mu_{\mathrm{Com}} - \mu_{\mathrm{Lib}}\bigg) =\\
&\bigg(\mu_{\mathrm{Com}} - \mu_{\mathrm{None}}\bigg) = \bigg(\mu_{\mathrm{Lib}} - \mu_{\mathrm{None}}\bigg) = 0
\end{split}
$$
...
$$
\begin{split}
\hat{\mathrm{Knowledge}_i} = &56.5 + 7.2(\mathrm{Com}_i) + 9.2(\mathrm{Lib}_i) -7.8(\mathrm{None}_i)
\end{split}
$$
...
$$
\begin{split}
\hat{\mathrm{Knowledge}_i} = &63.6 - 7.2(\mathrm{Con}_i) + 2.0(\mathrm{Lib}_i) -15.0(\mathrm{None}_i)
\end{split}
$$
...
#| fig-alt: "Pairwise comparisons of average teen substance use for different family structures. <br /><br />*Instructions:* Read across the row for a family structure to compare substance use with the family structures listed along the top of the chart. The symbols indicate whether the average substance use of the family structure in the row is significantly lower than that of the comparison family structure, significantly higher than that of the comparison family structure, or if there is no statistically discernible difference between the average substance use of the two countries."
#| out-width: "70%"
...
```{r}
#| label: tbl-mult-comp-2
#| echo: false
#| tbl-cap: "Pairwise contrasts between the average news knowledge for different news sources."
#| results: asis
data.frame(
  contrast = c("Comedy -  Conservative", "Comedy -  Liberal", "Comedy -  None", "Conservative -  Liberal", "Conservative -  None", "Liberal - None"),
  mean_diff = c("7.1", "-2.0", "15.0", "-9.1", "7.8", "16.9"),
  p = c("0.046", "0.598", "<.001", "<.001", "<.001", "<.001")
) |>
  gt() |>
  cols_align(
    align = "left",
...
$$
H_0: \mu_{\mathrm{Com}} = \mu_{\mathrm{Con}} = \mu_{\mathrm{Lib}} = \mu_{\mathrm{None}}
$$
...
$$
\begin{split}
\hat{\mathrm{Knowledge}_i} = &-27.80 + 0.19(\mathrm{Age}_i) + 3.24(\mathrm{Education}_i) + 0.23(\mathrm{News~Consumption}_i) + 0.23(\mathrm{Engagement}_i) +\\
& 7.87(\mathrm{Com}_i) + 0.08(\mathrm{Con}_i) +  5.96(\mathrm{Lib}_i)
\end{split}
$$
...
```{r}
#| label: tbl-mult-comp-3
#| echo: false
#| tbl-cap: "Pairwise contrasts between the average news knowledge for different news sources after controlling for differences in age, education, news consumption, and political engagement."
#| results: asis
data.frame(
  contrast = c("Comedy -  Conservative", "Comedy -  Liberal", "Comedy -  None", "Conservative -  Liberal", "Conservative -  None", "Liberal - None"),
  mean_diff = c("7.79", "1.91", "7.87", "-5.87", ".083", "5.96"),
  p = c(".011", ".548", ".010", ".001", ".955", "<.001")
) |>
  gt() |>
  cols_align(
    align = "left",
...
#| fig-alt: "95% confidence intervals for the adjusted mean news knowledge scores for Americans who get their news from eight different sources. The means are adjusted for differences in age, education level, amount of news consumed, and political engagement."
#| fig-width: 8
#| fig-height: 6
#| out-width: "80%"
# Create plot
