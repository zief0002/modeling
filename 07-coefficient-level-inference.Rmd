...
```{r fig.cap='Density plots of the marginal distributions of GPA and time spent on homework. The scatterplot showing the relationship between GPA and time spent on homework is also shown.'}
# Scatterplot
...
kable(table_01,
    caption = "Correlations between 8th-Grade students' GPA and weekly time spent on homework. Means and standard deviations (in parentheses) are displayed on the main diagonal.",
...
```{r echo=FALSE, out.width='80%', fig.cap='Thought experiment for sampling samples of size n from the population to obtain the sampling distribution of the mean.'}
include_graphics("figs/notes-06-thought-experiment-means.png")
```
...
```{r echo=FALSE, out.width='80%', fig.cap='Thought experiment for sampling samples of size n from the population to obtain the sampling distribution of the regression coefficients.'}
include_graphics("figs/notes-06-thought-experiment-coefficients.png")
```
...
```{r echo=FALSE, fig.cap='Sampling distribution of the slope coefficient. The distribution is approximately normal with a mean of 1.21 and a standard error of 0.35.', fig.width=6, fig.height=4}
...
```{r echo=FALSE, fig.cap='Comparison of two distributions. The normal distribution (solid, blue) and one with additional uncertainty (dashed, orange).', fig.width=6, fig.height=4}
ggplot(data.frame(x = c(-3, 3)), aes(x)) +
...
```{r echo=FALSE, fig.cap='The normal distribution (solid, blue) and two with additional uncertainty; one based on n=10 (dashed, orange), and the other based on n=4 (dotted, green).', fig.width=6, fig.height=4}
ggplot(data.frame(x = c(-3, 3)), aes(x)) +
  stat_function(fun = dnorm, color = "#1f77b4") +
  stat_function(fun = dt, args = list(df = 9), color = "#ff7f0e", linetype = "dashed") +
...
```{r echo=FALSE, fig.cap='Plot of the probability curve for the t(98) distribution. The shaded area under the curve represents the p-value for a two-tailed test evaluating whether the population slope is zero using an observed t-value of 73.42.', fig.width=6, fig.height=4}
new = data.frame(
...
include_graphics("figs/pvalue-and-tinder.jpg")
```
...
```{r echo=FALSE, fig.width=6, fig.height = 3, out.width='70%', fig.cap='Coefficient plot for the model regressing GPA on time spent on homework. Uncertainty based on the 95\\% confidence intervals are displayed.'}
# Load library
library(dotwhisker)
...
```{r fig.width=6, fig.height = 2, out.width='80%', fig.cap='Coefficient plot for the model regressing GPA on time spent on homework. Uncertainty based on the 95\\% confidence interval is displayed. Note that the intercept has been omitted.'}
# Omit intercept
mod_1 = tidy(lm.a) %>%
  mutate(model = "Model A") %>%
