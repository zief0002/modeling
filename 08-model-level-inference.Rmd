...
```{r echo=FALSE, out.width='80%', fig.cap='Thought experiment for sampling samples of size n from the population to obtain the sampling distribution of R-squared.'}
include_graphics("figs/notes-07-thought-experiment-r2.png")
```
...
```{r echo=FALSE, fig.width=8, fig.height=6, out.width='60%', warning=FALSE, cache=TRUE, fig.cap="Sampling distribution based on 1000 simple random samples of size 32 drawn from a population where $\\rho^2=0$."}
r2 = rep(NA, 1000)
...
```{r echo=FALSE, fig.cap='Plot of the probability curve for the F(1,98) distribution. The shaded area under the curve represents the *p*-value for a test evaluating whether the population rho-squared is zero using an observed *F*-value of 11.74.'}
new = data.frame(
...
```{r echo=FALSE, out.width='80%', fig.cap='Thought experiment for sampling samples of size *n* from the population to obtain the fitted regression line.'}
include_graphics("figs/notes-07-thought-experiment-confidence-envelope.png")
```

Now, imagine superimposing all of these lines on the same plot. 

```{r echo=FALSE, fig.cap='Plot showing the fitted regression lines for many, many random samples of size *n*.'}
include_graphics("figs/notes-07-superimposed-lines.png")
```
...
```{r fig.cap="GPA plotted as a function of time spent on homework. The OLS regression line (raspberry) and confidence envelope (grey shaded area) are also displayed."}
# Create plot
...
```{r echo=FALSE, fig.cap="GPA plotted as a function of time spent on homework. The OLS regression line (raspberry) and confidence ebnvelope (grey shaded area) are also displayed. The fitted value at *X*=6 is displayed as a point and the uncertainty in the estimate is displayed as an error bar."}
# Create plot
d = data.frame(
  x = 6,
  gpa = 81.57493,
