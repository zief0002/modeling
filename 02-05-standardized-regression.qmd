...
1. Standardize the outcome and predictor(s) by turning all the observations into *z*-scores
...
```{r fig.cap="Plot of the standardized income versus the standardized education level for the Riverview employees. The mean values are also displayed (dashed lines) along with the fitted regression line (solid line)."}
