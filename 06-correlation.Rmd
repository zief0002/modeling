...
```{r echo=FALSE, fig.cap="Hours of daylight versus day of the year for $n=75$ days in Minneapolis."}
# Reproduce sampling
set.seed(100)
...
```{r echo=FALSE, out.width="45%", fig.cap="Salary versus neuroticism (0 = not at all neurotic; 7= very neurotic) as measured by the Big Five personality survey for $n=1000$ employees from a Fortune 500 company. The second plot shows the same data colored by education level.", fig.show='hold'}
# Example 2
set.seed(123)
n = 1000
...
```{r echo=FALSE, fig.cap="Plot of the standardized GPA versus the standardized time spent on homework for the 100 observations. The mean values are also displayed."}
keith = keith %>%
  mutate(
    z_gpa = scale(gpa),
...
```{r echo=FALSE, fig.cap="Plot of GPA versus the time spent on homework (both unstandardized) for the 100 observations. The mean values are also displayed."}
ggplot(data = keith, aes(x = homework, y = gpa)) +
  geom_point() +
  theme_bw() +
